...
UPDATE CLIENTES SET desconto = 0
...
(
    id_produto INTEGER FOREIGN KEY REFERENCES PRODUTO,
    id_movimento INTEGER FOREIGN KEY REFERENCES MOVIMENTO,
    quantidade INTEGER
)
...
CREATE FUNCTION calcula_movimento
(
    @id_produto INTEGER,
    @quantidade INTEGER
)
RETURNS FLOAT -- Retorna o total do movimento
AS
BEGIN
    DECLARE @total_movimento FLOAT

    SELECT @total_movimento = preco * @quantidade
    FROM PRODUTO
    WHERE id_produto = @id_produto

    RETURN @total_movimento
END
...
AS
BEGIN
    DECLARE @media_acumulado FLOAT

    SELECT @media_acumulado = AVG(total_acumulado)
    FROM MOVIMENTO
    WHERE id_cliente = @id_cliente

    RETURN @media_acumulado
END
...
GROUP BY P.nome, P.preco, P.stock
HAVING P.stock < 5
...
BEGIN
    ...
    BEGIN
        ...
        IF @stock < @need
        BEGIN
            RAISERROR('Insufficiente stock. Only %d items left.', 16, 1, @stock); -- Raise an error, 16, 1 is the severity and state
...
CREATE TRIGGER atualiza_desconto
ON MOVIMENTO
AFTER UPDATE
AS
BEGIN
    ...
    SELECT @totalpaid = SUM(total_acumulado) FROM MOVIMENTO WHERE id_cliente = @clientId;

    -- Calculate total quantity of a specific product (assmung product ID is known, say 1)
    SELECT @totalQuantity = SUM(quantidade) FROM MOVIMENTO_PRODUTO
    JOIN MOVIMENTO ON MOVIMENTO_PRODUTO.id_movimento = MOVIMENTO.id_movimento
    WHERE MOVIMENTO.id_cliente = @clientId AND MOVIMENTO_PRODUTO.id_produto = 1;
    ...
    BEGIN 
        ...
        UPDATE CLIENTES SET desconto = 10 WHERE id_cliente = @clientId AND (desconto IS NULL OR desconto < 10);
    END
END;

-- Delete Trigger atualiza_desconto
DROP TRIGGER atualiza_desconto

-- Execução do Trigger
UPDATE MOVIMENTO SET total_acumulado = 7300 WHERE id_cliente = 1
...
CREATE PROCEDURE aplicaDesconto_set
(
    @desconto FLOAT,
    @limite FLOAT
)
...
BEGIN
    ...
    WHERE id_cliente IN(
        ...
        FROM MOVIMENTO
        GROUP BY id_cliente
        HAVING SUM(total_acumulado) > @limite
    )
